...
| Status | Meaning | Set By | Next States |
|--------|---------|--------|-------------|
| `CLAIMED` | Executor claimed ownership, working on it | Execution worker | `PLACED`, `FAILED`, `TIMEOUT` |
| `PLACED` | Order placed with broker, monitoring | Execution worker | `COMPLETED`, `FAILED`, `TIMEOUT` |
| `COMPLETED` | Execution finished (check execution_result) | Worker | *(terminal)* |
| `FAILED` | Execution failed (check execution_result) | Worker | *(terminal)* |
...
5. Success this time (still in CLAIMED state)
...
9. New executor continues monitoring
...
Every 5 seconds (per executor):
  ...
  2. For each slice:
     ...
     d. Create execution record:
        ...
        - execution_status = 'CLAIMED'
     e. Validate order parameters (while in CLAIMED state)
        - Check instrument, quantity, price, etc.
        - If validation fails: mark execution as FAILED with VALIDATION_FAILED result
     ...
     h. Update execution_status to 'PLACED'
     i. If market order: usually completes immediately, finalize
     j. If limit order: continue monitoring (stays in PLACED state)
```
...
Every 1 minute:
  ...
  2. For each timed-out execution:
     ...
     d. Check if broker_order_id exists:
        - If YES: Another executor can pick up and continue monitoring
...
| `order_slice_executions` | `execution_status` | Simple execution state | CLAIMED, PLACED, COMPLETED, FAILED, TIMEOUT |
