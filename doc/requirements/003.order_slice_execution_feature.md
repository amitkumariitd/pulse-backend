...
CREATE INDEX idx_broker_events_attempt ON order_slice_broker_events(execution_id, attempt_number);
```
...
Every 5 seconds (per executor):
  ...
  2. For each slice:
     ...
     e. Attempt to place order with Zerodha (with retry logic):
        ...
        - Record broker event (event_type='PLACE_ORDER', attempt_number=placement_attempts)
...
After placing order (execution_status = 'PLACED'):
  ...
  2. If limit order:
     ...
     - Every 5 seconds:
       ...
       d. Record broker event (event_type='STATUS_POLL', attempt_number=1)
...
1. **Claim Ownership:**
   - Executor sets `executor_id`, `executor_claimed_at`
