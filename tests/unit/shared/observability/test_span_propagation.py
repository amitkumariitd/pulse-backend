...
def test_span_id_becomes_parent_span_id_across_services():
    """Test that incoming X-Span-Id becomes parent_span_id in receiving service."""
    ...
    # Act - Send request with X-Span-Id header (simulating call from GAPI)
    headers = {
        ...
        "X-Span-Id": "sa1b2c3d4",  # GAPI's span_id
    ...
    # Verify parent_span_id is set from incoming X-Span-Id
    ...
    # Verify span_source was built by appending to parent
    assert captured_context.span_source == "GAPI:POST/api/orders->PULSE:GET/test"


def test_no_parent_span_id_when_first_service():
    """Test that parent_span_id is None when no X-Span-Id header is present."""
    # Arrange
    app = FastAPI()
    ...
    # Act - Send request without X-Span-Id header (first service in chain)
...
def test_span_source_chain_building():
    ...
    headers = {
        ...
        "X-Span-Id": "sb2c3d4e5",  # PULSE's span_id
    ...
    # Verify span_source shows full chain
    assert captured_context.span_source == "GAPI:POST/api/orders->PULSE:POST/internal/orders->BROKER:POST/submit"
...
def test_response_headers_include_new_span_id():
    ...
    headers = {
        "X-Span-Id": "sa1b2c3d4"  # Parent span
    }
